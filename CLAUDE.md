...
- **Secrets come from SSM Parameter Store, fetched at Lambda cold start** by `_bootstrap_ssm_secrets` in `main.py` (not via CloudFormation dynamic refs, so version pins don't apply). Rotating a secret means overwriting the SSM value in place; the next cold start picks it up. Locally, the same names come from `.env`.
